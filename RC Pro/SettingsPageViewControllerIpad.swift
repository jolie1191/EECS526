//
//  SettingsPageViewControllerIpad.swift
//  RC Pro
//
...
class SettingsPageViewControllerIpad: UIViewController {
...
//    var currentDevice: CBPeripheral! = nil
 ...
 /*
    @IBAction func unwindToMain(segue: UIStoryboardSegue) {
        self.performSegueWithIdentifier("unwindToMainFromSettings", sender: self)
    }
 */   
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if let relaySettingsPage = segue.destinationViewController as? RelaySettingsViewControllerIpad {
    ...
    override func viewDidLayoutSubviews() {
        self.scrollView.contentSize = CGSizeMake(364, 851)
    }
    
    private func loadUserDefaults() {
        print("load user Default")
        self.controlCenterSegment.selectedSegmentIndex = globalDefaults.integerForKey("controlCenterType")
        self.txtPasscode.text = globalDefaults.stringForKey("passcode")
/*
        if ble.activePeripheral != nil && ble.activePeripheral.state == CBPeripheralState.Connected && currentDevice != nil {
            if globalDefaults.stringForKey("deviceName" + currentDevice.identifier.UUIDString) != nil {
                txtControlCenterName.text = globalDefaults.stringForKey("deviceName" + currentDevice.identifier.UUIDString)
            }
            else {
                txtControlCenterName.text = "RC Center"
            }
        }
        else {
            labelControlCenterName.hidden = true
            txtControlCenterName.hidden = true
            
            var frameRect: CGRect = txtPasscode.frame;
            frameRect.size.width = 200
            txtControlCenterName.frame = frameRect;
        }
*/
...
/*
        if ble.activePeripheral != nil && ble.activePeripheral.state == CBPeripheralState.Connected && currentDevice != nil {
            ...
            if globalDefaults.stringForKey("relay1Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings1.text = globalDefaults.stringForKey("relay1Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay2Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings2.text = globalDefaults.stringForKey("relay2Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay3Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings3.text = globalDefaults.stringForKey("relay3Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay4Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings4.text = globalDefaults.stringForKey("relay4Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay5Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings5.text = globalDefaults.stringForKey("relay5Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay6Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings6.text = globalDefaults.stringForKey("relay6Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay7Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings7.text = globalDefaults.stringForKey("relay7Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay8Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings8.text = globalDefaults.stringForKey("relay8Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay9Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings9.text = globalDefaults.stringForKey("relay9Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay10Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings10.text = globalDefaults.stringForKey("relay10Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay11Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings11.text = globalDefaults.stringForKey("relay11Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay12Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings12.text = globalDefaults.stringForKey("relay12Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay13Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings13.text = globalDefaults.stringForKey("relay13Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay14Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings14.text = globalDefaults.stringForKey("relay14Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay15Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings15.text = globalDefaults.stringForKey("relay15Name" + currentDevice.identifier.UUIDString)
            }
            
            if globalDefaults.stringForKey("relay16Name" + currentDevice.identifier.UUIDString) != nil {
                self.btnRelaySettings16.text = globalDefaults.stringForKey("relay16Name" + currentDevice.identifier.UUIDString)
            }
        }
*/
    }
...
/*
        // Save device-specific relay name while connected
        if ble.activePeripheral != nil && ble.activePeripheral.state == CBPeripheralState.Connected && currentDevice != nil {
            globalDefaults.setObject(self.btnRelaySettings1.text, forKey: "relay1Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings2.text, forKey: "relay2Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings3.text, forKey: "relay3Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings4.text, forKey: "relay4Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings5.text, forKey: "relay5Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings6.text, forKey: "relay6Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings7.text, forKey: "relay7Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings8.text, forKey: "relay8Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings9.text, forKey: "relay9Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings10.text, forKey: "relay10Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings11.text, forKey: "relay11Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings12.text, forKey: "relay12Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings13.text, forKey: "relay13Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings14.text, forKey: "relay14Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings15.text, forKey: "relay15Name" + currentDevice.identifier.UUIDString)
            globalDefaults.setObject(self.btnRelaySettings16.text, forKey: "relay16Name" + currentDevice.identifier.UUIDString)
            
            // Save customized device name
            globalDefaults.setObject(self.txtControlCenterName.text, forKey: "deviceName" + currentDevice.identifier.UUIDString)
        }
*/
